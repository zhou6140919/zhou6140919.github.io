...
description: The authors develop a dual-learning mechanism, which can enable an NMT system to automatically learn from unlabeled data through a dual-learning game.
---
...
Solution: The authors develop a dual-learning mechanism, which can enable an NMT system to automatically learn from unlabeled data through a dual-learning game.

Explanation: Any machine translation task has a dual task; the primal and dual tasks can form a closed loop, and generate informative feedback signals to train the translation models, even if without the involvement of a human labeler. In the dual-learning mechanism, the authors use two agents to represent the model of the primal task and the dual task respectively, then ask them to teach each other through a reinforcement learning process. Based on the feedback signals (the language model likelihood of the output and the reconstruction error of the original sentence after the primal and dual translations), we can iteratively update the two models until convergence. The authors call this approach dual-NMT.
...
Assume the authors already have two well-trained language models $$LM_A$$ and $$LM_B$$, each of which takes a sentence as input and outputs a real value to indicate how confident the sentence is a natural sentence in its own language. Here the language models can be trained either using other resources, or just using the monolingual data $$D_A$$ and $$D_B$$.
...
The authors use beam search to obtain more meaningful results for gradient computation, i.e., the authors greedily generate top-K high-probability middle translation outputs, and use the averaged value on the beam search results to approximate the true gradient.

In order to smoothly transit from the initial model trained from bilingual data to the model training purely from monolingual data, the authors adopted the following soft-landing strategy.

At the very beginning of the dual learning process, from each mini batch, the authors used half sentences from monolingual data and half sentences from bilingual data. The objective was to maximize the reward based on monolingual data and bilingual data. When the training process went on, we gradually increased the percentage of monolingual sentences in the mini batch, until no bilingual data were used at all. Specifically, we tested two settings.
 - In the first setting (referred to Large), the authors used all the 12M bilingual sentences pairs during the soft-landing process. That is, the warm start model was learnt based on full bilingual data.
 - In the second setting (referred to Small), the authors randomly sampled 10% of the 12M bilingual sentences pairs and used them during the soft-landing process.


## Discussions

1. The basic idea of dual learning is generally applicable: as long as two tasks are in dual form, the authors can apply the dual-learning mechanism to simultaneously learn both tasks from unlabeled data using reinforcement learning algorithms.
...
2. The key idea is to form a closed loop so that the authors can extract feedback signals by comparing the original input data with the final output data. Therefore, if more than two associated tasks can form a closed loop, we can apply our technology to improve the model in each task from unlabeled data. (closed-loop learning perhaps)

## To explore in the future

1. In the experiments the authors used bilingual data to warm start the training of dual-NMT. A more exciting direction is to learn from scratch, i.e., to learn translations directly from monolingual data of two languages (maybe plus lexical dictionary).
